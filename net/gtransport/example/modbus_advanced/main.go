...
func runAdvancedExample() error {
	...
	value, err := parseSingleRegisterTCPResponse(respFrame, 0x0001, 0x11)
	if err != nil {
		return err
	}
...
func parseSingleRegisterTCPResponse(frame []byte, transactionID uint16, slaveID byte) (uint16, error) {
	if len(frame) < 7 {
		return 0, fmt.Errorf("unexpected tcp response length %d", len(frame))
	}
	if gotTransactionID := binary.BigEndian.Uint16(frame[0:2]); gotTransactionID != transactionID {
		return 0, fmt.Errorf("unexpected transaction id 0x%04x", gotTransactionID)
	}
	if frame[6] != slaveID {
		return 0, fmt.Errorf("unexpected slave id 0x%02x", frame[6])
	}
